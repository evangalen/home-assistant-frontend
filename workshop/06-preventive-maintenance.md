...
improve on the Developer eXperience (DX) site:
...
fix bugs causes by the missing `type` property for the `@property` decorators / annotations

So let us do some Preventive Maintenance and change our ast-grep search rule into a lint rule.
And then we can spice things up by also including an autofix for the lint rule.

### Creating an ast-grep lint rule

Since we also have our ast-grep search rule, we can change it into an ast-grep lint rule with little effort:

- we need to come up with a new `id` and file name for the lint rule instead of the with `find-`.. one we used for the search rule
- we need to specify a `message` that will be displayed whenever the lint rule fails
- we need to specify a `severity`, other than the default of `hint`, like `error` or `warning`

Then copy the `find-boolean-lit-property-without-type.yml` of `ast-grep/rules/search` to `ast-grep/rules/lint` and
change the file name to `missing-boolean-lit-property-type.yml`.

Now than the `id` in the new file and also add a `message` and `severity` to the new file:
...
Notice that the earlier search results are now shown in red, due to the `severity: error`, and a message is display for
each result.
...
`decorator` AST node that's located inside the `public_field_definition` AST node.

And, to keep things simple, we'll later split the rule into two rules that we will separate using the `---` YAML marker
to create a multi-document YAML file.

But, for now first, and relatively simple, part of the lint rule.
...
For now, we'll not apply to autofix changes suggested by ast-grep, because our lint rule is only half complete yet.

### Changing lint rule into a multi-document YAML file 
To also support an autofix for `property` decorators / annotations, we need to change our YAML file into a
multi-document YAML file.

For this we need to add a `---` YAML marker after the first and then created an extra lint rule after that.

When we would also change the target AST node to the `decorator` AST node, the contents of the YAML file would look like
this (omitting most of what's before the `---` YAML marker):
```yaml
id: missing-boolean-lit-property-type
severity: error
message: "Missing `type` property inside options of `@property` decorator of boolean class field."
...
To capture (zero or more) object pairs, we use a meta-variable with `$$$` (triple `$`) prefix in the `pattern` entry:
```yaml
pattern: |
@property({ $$$PAIRS })
```

Besides the `pattern` entry from above, we still also need the existing `pattern` with the `@property($PROPERTY_OPTIONS)`
code pattern.

For this we'll a `and` entry, of the Relational Rules of ast-grep, to combine the two patterns:
```yaml
# ..
rule:
  all:
...
part, we'll also need to explicitly exclude:
```yaml
# ..
rule:
  all:
...
Which makes the contents of the `missing-boolean-lit-property-type.yml` file to be like this:
```yaml
# yaml-language-server: $schema=https://raw.githubusercontent.com/ast-grep/ast-grep/main/schemas/rule.json
id: missing-boolean-lit-property-type
language: ts
...
To interactively apply autofix-es, ast-grep offers a nice interactive mode through the `--interactive` CLI flag:
```shell
...
Using interactive mode, you can either selectively apply the autofix suggestions, using [y]es and [n]o keys, or you can
choose to be yolo mode and use the [a]ll key to simply apply all autofix suggestions.

Since life is so short to selectively apply the autofix suggestions, we'll just use the [a]ll key to apply all autofix
suggestions.
